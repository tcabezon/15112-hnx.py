...
def appStarted(app):
    ...
    app.mode='mainMode'
    app.ring=Ring(200,5,stepRing=28,stepCircunference=28)
    app.draw=True
    ...
    if ((mousePosition[0])<app.width*app.meshViewStart and (mousePosition[1])<app.width*app.meshViewStart ):
        ...
        if mousePressedInsideRing(app,mousePosition):
            if app.drawElement=='Sphere':
                ...
                newSphere=Sphere(r,cx=0,cy=mousePosition[0],cz=mousePosition[1],stepU=25,stepV=36)
                print(newSphere)
                ...
                newBezier=Bezier3D(scaledControlPoints,app.numberOfBezierPoints,cx=0,cy=mousePosition[0],cz=mousePosition[1],stepU=36)
                print(newBezier)

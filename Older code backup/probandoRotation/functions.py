...
def printModeDictionary(app, canvas):
    ...
    canvas.create_text(app.margin+(app.meshViewStart*app.width*app.doubleView), app.margin,
                       text=resultString, font='Arial 12 bold', anchor='nw')
    
    canvas.create_text(app.margin+(app.meshViewStart*app.width*app.doubleView), app.height-app.margin,
                       text=f'{app.mode}', font='Arial 12 bold', anchor='sw')
...
def displayFaces2D(app,canvas):
    ...
    for i in range(app.ring.faces.shape[0]):
        ...
        #print(f'face {i}: ',faceIndexes)
        center=[(app.doubleView*app.width*app.meshViewStart)+app.width/2,app.height/2]
...
def convertEditViewToBezierPoints(app,point):
    newPointX=(app.meshViewStart*app.width*app.doubleView)/2-point[1]
    newPointY=point[0]-(app.meshViewStart*app.width*app.doubleView)/2
